...
const startUI = () => {
  ...
  const clearTimer = createTimerFn(() => {
    ...
    createRoot(rootEl).render(
      <TransactionExplanation tx={txHash} chain={CHAIN.chain} clearTimer={clearTimer} />
    )
  })({ clearFn: () => { return !(txHash === txPage.hash && GLOBAL.isTx) } })
}
...
const genTransactionExplanationBtn = () => {
  createTimerFn(() => {
    ...
    createRoot(rootEl).render(<ExplainBtn onClick={() => { startUI()}} />)
  })()
}
