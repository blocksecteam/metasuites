...
const renderDrawer = ({
  ...
  const connectStatus = $(`#${id}`).contents().find('#connectStatus')
...
export const genSimulateBtn = async (chain: string) => {
  ...
  writeContractIframes.each(function () {
    ...
    const renderSimulateButtons = () => {
      const isOptimism = !$(this).contents().find('#header > div > span').length
      if (isOptimism) {
        // optimistic
        $(this).contents().find('#header > div').append('<span></span>')
        $(this)
          .contents()
          .find('#header > div > a')
          .appendTo($(this).contents().find('#header > div > span'))
      }
      $(this)
        .contents()
      ...
      $(this)
        ...
        .each(function () {
          ...
          const funcName = cardJquery
            .find('.card-header > a[data-parent="#accordion"]')
            .text()
            .split(' ')[1]
          const rootEl = document.createElement('span')
