      ...
      Array.from(colEls).forEach((td, idx) => {
        if (!uselessCols.includes(idx)) {
          if (idx === fromIdx || idx === toIdx) {
            const tagEl = td.querySelector<HTMLElement>('.hash-tag')
            const dataOriginTitle =
              tagEl?.getAttribute('data-original-title') ??
              tagEl?.getAttribute('title') ??
              ''
            const content = (tagEl?.innerText ?? '').trim()
            const label = content
            const address = isAddress(content)
              ? content
              : pickAddress(dataOriginTitle) ?? ''
            if (label) {
              if (idx === fromIdx) {
                fromLabel = label
              } else {
